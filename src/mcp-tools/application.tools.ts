...
export function createApplicationTools(): MCPToolHandler[] {
  return [
...

    {
      ...
      description: 'Get detailed application information by ID (GUID) or name',
      schema: {
      ...
      handler: async (args: any, context: ToolContext): Promise<ToolResponse> => {
        try {
          ...
          if (isGuid(args.application)) {
            ...
            result = await context.veracodeClient.getApplicationDetails(args.application);
          } else {
            // Handle as application name
            result = await context.veracodeClient.getApplicationDetailsByName(args.application);
          }
