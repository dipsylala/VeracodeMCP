  ...
  return [
    {
      ...
      handler: async (args: any, context: CLIToolContext): Promise<ToolResponse> => {
        ...
        const result = await context.veracodeClient.findings.getStaticFlawInfo(args.app_id, args.issue_id, args.sandbox_id);
        return {
          success: true,
          data: result
        };
...

    {
      ...
      handler: async (args: any, context: CLIToolContext): Promise<ToolResponse> => {
        ...
        const result = await context.veracodeClient.findings.getStaticFlawInfoByName(args.name, args.issue_id, args.sandbox_id);
        return {
          success: true,
          data: result
        };
