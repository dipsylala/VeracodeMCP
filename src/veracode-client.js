...
export class VeracodeClient {
     ...
     */
    async generateVeracodeAuthHeader(url, method) {
     ...
     */
    async getApplicationDetailsByName(name) {
        try {
     ...
     */
    async getScanResults(appId, scanType) {
        try {
     ...
     */
    async getFindings(appId, options) {
        try {
            let url = `appsec/v2/applications/${appId}/findings`;
            const params = new URLSearchParams();
            if (options) {
                if (options.scanType)
                    params.append("scan_type", options.scanType);
                if (options.severity !== undefined)
                    params.append("severity", options.severity.toString());
                if (options.severityGte !== undefined)
                    params.append("severity_gte", options.severityGte.toString());
                if (options.cwe && options.cwe.length > 0) {
                    options.cwe.forEach(cweId => params.append("cwe", cweId.toString()));
                }
                if (options.cvss !== undefined)
                    params.append("cvss", options.cvss.toString());
                if (options.cvssGte !== undefined)
                    params.append("cvss_gte", options.cvssGte.toString());
                if (options.cve)
                    params.append("cve", options.cve);
                if (options.context)
                ...
                if (options.mitigatedAfter)
                    params.append("mitigated_after", options.mitigatedAfter);
                if (options.newFindingsOnly !== undefined)
                    params.append("new", options.newFindingsOnly.toString());
                if (options.scaDependencyMode)
                    params.append("sca_dep_mode", options.scaDependencyMode);
                if (options.scaScanMode)
                    params.append("sca_scan_mode", options.scaScanMode);
                if (options.policyViolation !== undefined)
                    params.append("violates_policy", options.policyViolation.toString());
     ...
     */
    async getFindingsByName(name, options) {
        try {
            // First search for applications with this name
            const searchResults = await this.searchApplications(name);
            if (searchResults.length === 0) {
     ...
     */
    async getPolicyCompliance(appId) {
        try {
