...
class RawVeracodeClient {
    ...
    async getScanResults(appId) {
        try {
            const response = await this.apiClient.get(`appsec/v1/applications/${appId}/scans`);
...
async function inspectRawResponse() {
    try {
        const client = new RawVeracodeClient(
            process.env.VERACODE_API_ID,
            process.env.VERACODE_API_KEY
        );

        console.log("Raw scan results (without URL conversion):");
        const result = await client.getScanResults("2d8925b2-ce45-431a-8c0e-e94dd4376750");
