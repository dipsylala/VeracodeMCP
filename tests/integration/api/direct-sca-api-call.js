...
async function makeDirectScaCall() {
    ...
    const APP_ID = '65c204e5-a74c-4b68-a62a-4bfdc08e27af'; // MCPVerademo-Net
    ...
    console.log(`📡 Target URL: ${apiBaseUrl}appsec/v2/applications/${APP_ID}/findings`);
    console.log(`📋 Query: scan_type=SCA&size=10`);

    try {
        // Create the URL and method for signing
        const method = 'GET';
        const url = `appsec/v2/applications/${APP_ID}/findings?scan_type=SCA&size=10`;
        const fullUrl = `${apiBaseUrl}${url}`;

        console.log(`\n🔐 Generating HMAC-SHA256 signature...`);
        const authData = auth.generateAuthHeader(
            `/appsec/v2/applications/${APP_ID}/findings?scan_type=SCA&size=10`,
            method,
            'api.veracode.com'
        );
